<div class="bg-gray-900 border border-gray-800 rounded-xl p-4 shadow-sm hover:shadow-md transition flex flex-col justify-between theme-card">
    <div>
        ...
        <div class="flex justify-between items-center mb-3">
            <div class="flex items-center gap-2">
                ...
                <!-- نموذج الحذف المباشر -->
                <form action="{{ route('orders.destroy', $order->id) }}" method="POST" onsubmit="return confirm('هل أنت متأكد من حذف هذا الطلب نهائياً؟');" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit"
        ...
        <div class="text-xs text-gray-300 space-y-1 mb-3 bg-gray-950/60 p-2.5 rounded-lg border border-gray-800">
    ...
    <div>
        ...
        <form action="{{ route('orders.updateStatus', $order->id) }}" method="POST">
            @csrf
            @method('PATCH')
            <select name="status" onchange="this.form.submit()" class="w-full bg-gray-950 border border-gray-700 text-gray-200 text-xs rounded-lg p-1.5 focus:ring-1 focus:ring-amber-500 focus:outline-none">
